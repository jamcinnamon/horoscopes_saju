"""
자동 사주 계산 운세 생성 - PDF 버전
생년월일과 시간만 입력하면 사주를 자동으로 계산하고 PDF로 생성합니다.

📝 사용 방법:
1. 고객에게 생년월일, 출생시간만 받기
2. 아래 정보 입력
3. python auto_fortune_pdf.py 실행
4. 자동 계산된 사주 확인 (수정 가능)
5. 생성된 PDF 파일을 고객에게 전달
"""
import sys
import asyncio
...
# 기본 정보
NAME = "김희정"  # 이름 (선택사항)
BIRTH_DATE = "1993-05-20"  # 생년월일 (YYYY-MM-DD 형식)
BIRTH_TIME = "02:10"  # 출생 시간 (HH:MM 형식) - 모르면 빈칸 ""
